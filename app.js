(() => {
  ...
  function metadata(info) {
    ...
    if (canonical) canonical.href = new URL(C.href(info.route), config.siteUrl).href;
  }
  ...
  function navigate(route, { push = false, focus = false, restore = false } = {}) {
    ...
    metadata(next);
    if (next.view === 'home') { ensureScene(); scene.setActive(true); }
    ...
    requestAnimationFrame(() => {
      if (next.view === 'home' && current?.view === 'home') { scene.resize(); scene.render(performance.now()); }
    });
  }
